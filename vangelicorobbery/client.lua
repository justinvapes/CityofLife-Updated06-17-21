...
RegisterCommand('CloseStore', function(source, args, rawCommand)
	if Config.AllowPoliceStoreClose then
		...
		if PlayerData.job.name == 'security' then ispolice = true else
		...
		elseif ispolice then
			exports['mythic_notify']:SendAlert('inform', 'You must be standing near door to force the store closed.')
		end
	end
end)
	...
	while true do 
		...
		while storeclosed do
			...
			if GetDistanceBetweenCoords(plyloc, -622.2496, -230.8000, 38.05705, true)  < 10.0 then
				...
				DoScreenFadeIn(1250)
				exports['mythic_notify']:SendAlert('inform', 'Vangelico Jewelry is now Closed.')
			end
			Citizen.Wait(0)
			sleep = 0
			freezedoors(true)	
		...
		while policeclosed do
			...
			if GetDistanceBetweenCoords(plyloc, -622.2496, -230.8000, 38.05705, true)  < 10.0 then
				...
				SetEntityCoords(PlayerPedId(), -673.1831, -227.6621, 36.11,0,0,0,true)
				DoScreenFadeIn(1250)
				exports['mythic_notify']:SendAlert('error', 'Vangelico Jewelry is now Closed for cleanup!.')
			end
			Citizen.Wait(0)
			sleep = 0
			freezedoors(true)	
