        ...
        ESX.TriggerServerCallback("loaf_housing:getHouseData", function(data) 
            if data then
                ...
                if Config.Blips.IKEA then
                    AddBlip(Config.IKEA.EnterIKEA.xyz, 605, 0, Strings["ikea"])
                end
                    ...
                    for k, v in pairs(Config.Houses) do
                        ...
                        if Cache.owned_houses[sk] then
                            if Config.Blips.Owned then
                                Cache["blip_house_" .. sk] = AddBlip(v.Entrance.xyz, 40, 2, Strings["owned_" .. v.Type])
                            end
                        elseif Cache.bought_houses[sk] then
                            if houseKeys["house_" .. sk] and Config.Blips.Access then
                                Cache["blip_house_" .. sk] = AddBlip(v.Entrance.xyz, 40, 7, Strings["access_" .. v.Type])
                            elseif Config.Blips.Others then
                                Cache["blip_house_" .. sk] = AddBlip(v.Entrance.xyz, 40, 3, Strings["purchased_" .. v.Type])
                            end
                        elseif ESX.GetPlayerData().job.name == 'rea' then
                            if Config.Blips.Buyable then
                                Cache["blip_house_" .. sk] = AddBlip(v.Entrance.xyz, 40, 0, Strings["purchasable_" .. v.Type])
                            end
                        end
                        Wait(5)
                    end
