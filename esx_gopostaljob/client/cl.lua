...
function CreateBlip()
    if PlayerData.job ~= nil and PlayerData.job.name == 'gopostal' then
		...
		if BlipCloakRoom == nil then
			...
			AddTextComponentString('(1) Work Vehicle')
		...
		if BlipPickRoom == nil then
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString('(2) Loading Van')
...
Citizen.CreateThread(function()
 while true do
  Citizen.Wait(5)
  if PlayerData.job ~= nil and PlayerData.job.name == 'gopostal' then
   if(GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -414.01, 6171.50, 31.532, true) < 15) then
	DrawMarker(23, -414.01, 6171.50, 31.532-0.9, 0, 0, 0, 0.0, 0, 0, 3.0, 3.0, 0.5001, 255, 255, 255, 0, 0, 0, 0, 0)
   	if(GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -414.01, 6171.50, 31.532, true) < 1.5) then
   	 if DoesEntityExist(goPostalVehicle) then DrawText3d(-414.01, 6171.50, 31.532+0.5, "Press [~ r ~ E ~ s ~] to [~ r ~ Cancel ~ s ~] as [~ r ~ Deliveries ~ s ~]", 200) else DrawText3d(-414.01, 6171.50, 31.532+0.5, "Press [~ g ~ E ~ s ~] for [~ g ~ Start ~ s ~] as [~ g ~ Deliveries ~ s ~]", 200) end	
	   ...
	   RemoveJobBlip()
		...
		TriggerEvent("diablo_notes:sendNotification", "It will give [~ g ~ Restock to the Van ~ s ~]", 5000)
      end
     end
	end
   end
   if(GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -441.282, 6142.714, 31.478, true) < 40) and IsVehicleModel(GetVehiclePedIsIn(PlayerPedId(), true), GetHashKey("boxville4")) and DoesEntityExist(goPostalVehicle) then
   	DrawMarker(23, -441.282, 6142.714, 31.478-0.95, 0, 0, 0, 0.0, 0, 0, 3.0, 3.0, 0.5001, 255, 255, 255, 0, 0, 0, 0, 0)
	if(GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -441.282, 6142.714, 31.478, true) < 4) then
	 if not onJob then 
	  DrawText3d(-441.282, 6142.714, 31.478+1.2, "Press [~ g ~ E ~ s ~] to give [~ g ~ Restock to the Van ~ s ~]", 200)
   ...
   if(GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), currentJob[1], currentJob[2], currentJob[3], true) < 20) and onJob then
   	DrawMarker(1, currentJob[1], currentJob[2], currentJob[3], 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 112, 100,0, true, true, 0,0)
	if(GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), currentJob[1], currentJob[2], currentJob[3], true) < 1.5) then
	 drawTxt('Press [~g~ E ~s~] to [~g~ Deliver ~s~] to [~g~ Box ~s~]')
	 if IsControlJustPressed(0, 38) then
   ...
   if onJob and not IsPedInAnyVehicle(PlayerPedId()) and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), currentJob[1], currentJob[2], currentJob[3], true) < 20 then 
    local bootPos = GetEntityCoords(goPostalVehicle)
    if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), bootPos.x, bootPos.y, bootPos.z, true) < 2.5 and PackageObject == nil then 
	 drawTxt('Press [~g~ E ~s~] to [~g~ Pick up ~s~] in the [~g~ Box ~s~]')
...
function restockVan()
 ...
 while restockVan do 
  ...
  for id,v in pairs(restockObjectLocation) do 
   ...
   if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), v[1],v[2],v[3], true) < 1.0 then
	drawTxt('Carrega [~g~ E ~s~] para [~g~Pegar~s~] na [~g~Caixa~s~]')
    if IsControlJustPressed(0, 38) then 
  ...
  if carryingPackage.status then 
   local bootPos = GetEntityCoords(goPostalVehicle)
   if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), bootPos.x, bootPos.y, bootPos.z, true) < 2.6 then 
    drawTxt('Carrega [~g~ E ~s~] para [~g~Meter~s~] a [~g~Caixa na Carrinha~s~]')
    if IsControlJustPressed(0, 38) then 
     ...
     if restockPackages == 0 then 
      ...
      newShift()
      onJob = true
	  TriggerEvent("diablo_notes:sendNotification", "[~ y ~ Loaded Van ~ s ~] [~ g ~ You can start the job ~ s ~]", 5000)
     end
    end
   end
  end
...
::start::
 ...
 if id == 1 then 
  ...
  if currentJob[1] == jobLocation[1] and currentJob[2] == jobLocation[2] then goto start end
 ...
 currentJobPay = CalculateTravelDistanceBetweenPoints(GetEntityCoords(PlayerPedId()), currentJob[1],currentJob[2],currentJob[3])/2/4
 if currentJobPay > 600 then 
  currentJobPay = math.random(65, 120)
 end
end
