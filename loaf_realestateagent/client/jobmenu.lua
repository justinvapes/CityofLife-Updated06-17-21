        ...
        if reason then
            ...
            ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "billing_amount", {
                ...
                if amount and amount > 0 then
                    ...
                    if plr == -1 or dist > 3.0 then
                        if Config.AllowTransferSelf then
                            TriggerServerEvent("esx_billing:sendBill", GetPlayerServerId(PlayerId()), "society_" .. Config.Job.Name, reason, amount)
                            ESX.ShowNotification(Strings.Billed:format(tostring(amount), reason))
                        else
                            ESX.ShowNotification(Strings.NooneNearby)
                        end
                    else
                        TriggerServerEvent("esx_billing:sendBill", GetPlayerServerId(plr), "society_" .. Config.Job.Name, reason, amount)
                        ESX.ShowNotification(Strings.Billed:format(tostring(amount), reason))
                    end
                else
