...
Citizen.CreateThread(function()
	while true do
		...
		if cancelTimer > 0 then cancelTimer = cancelTimer - 1 end
		if IsControlJustPressed(0, 73) and cancelTimer < 1 then
			local plyPed = PlayerPedId()
			if DoesEntityExist(plyPed) and not IsEntityDead(plyPed) and not exports['mythic_hospital']:playerInBed() then
				if not IsPedSwimming(plyPed) and not IsPedShooting(plyPed) and not IsPedClimbing(plyPed) and not IsPedCuffed(plyPed) and not IsPedDiving(plyPed) and not IsPedFalling(plyPed) and not IsPedJumpingOutOfVehicle(plyPed) and not IsPedInParachuteFreeFall(plyPed) then
					if IsInAnimation or IsPedUsingAnyScenario(plyPed) then
						EmoteCancel()
					else
						EmoteCommandStart(nil, {'handsup', 0})
						local cancelTimer = 500
					end
				end
			end
		end
